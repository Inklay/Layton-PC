...
public:
	...
	struct Part {
		Part(uint16_t posX, uint16_t posY, uint16_t width, uint16_t height, fileUtils::buffer data) : m_posX(posX), m_posY(posY), m_width(width), m_height(height), m_data(data) {};
		uint16_t m_posX;
		uint16_t m_posY;
		uint16_t m_width;
		uint16_t m_height;
