...
namespace BG {
	...
	void convertToPng(const fileUtils::path& filePath, const fileUtils::buffer& buffer) {
#if _DEBUG
		//fileUtils::writeBin(buffer, filePath.parent_path() / filePath.filename().replace_extension("bg"));
#endif
		size_t offset = 0;
		...
		fileUtils::path outputFile = filePath.parent_path() / filePath.filename().replace_extension("png");
