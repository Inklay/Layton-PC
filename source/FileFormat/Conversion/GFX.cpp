...
void GFX::convertToPngs(const fileUtils::path& filePath, const fileUtils::path& outputFolder, bool isCompressed) {
	...
	if (isArj) {
		paletteCount = (buffer.at(offset + 3) << 24) | (buffer.at(offset + 2) << 16) | (buffer.at(offset + 1) << 8) | buffer.at(offset);
		offset += 4;
	}

	std::vector<GFX::Image> images = getImages(imageCount, colorDepth, buffer, offset, isArj);
	...
	for (unsigned int i = 0; i < paletteCount; i++) {
		colors.emplace_back((buffer.at(offset + 1 + i * 2) << 8) | buffer.at(offset + i * 2));
	}

	offset += paletteCount * 2 + 0x1E;

	createPngs(imageCount, colorDepth, colors, images, filePath, outputFolder, isArj);
	if (images.size() > 1) {
...
void GFX::createPngs(unsigned int imageCount, unsigned int colorDepth, const std::vector<uint16_t>& colors, std::vector<GFX::Image>& images, const fileUtils::path& filePath, const fileUtils::path& outputFolder, bool isArj) {
	for (unsigned int i = 0; i < imageCount; i++) {
		...
		for (GFX::Part& part : images.at(i).m_parts) {
			...
			if (isArj) {
				createArjPng(part, colors, size, pixelBuffer);
			} else {
				createArcPng(part, colors, size, pixelBuffer);
			}
		}
